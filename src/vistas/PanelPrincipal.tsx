...
import { Star, ThumbsUp, ThumbsDown, MessageSquare, TrendingUp, Package, ChevronDown, PlusCircle } from 'lucide-react';
import type { ProductoAnalizado } from '../tipos/contratos';
import ModalCargarProducto from '../componentes/ui/ModalCargarProducto';
...
export default function PanelPrincipal() {
  ...
  // Estados para gestionar el modal y el selector de productos
  ...
  // 1. Extraemos la lógica a una función para poder reutilizarla al cargar un nuevo producto
  ...
  // Función para recargar el panel cuando el modal termine
  ...
  // 2. Modificamos el bloque de error para incluir el botón discreto y el modal
  if (error || !datos) return (
    <>
      <div className="flex items-center justify-center h-full min-h-[400px]">
        <div className="text-neutral-500 text-sm bg-[#181818] border border-neutral-800 rounded-xl p-8 max-w-md mx-auto text-center space-y-4">
          <Package className="mx-auto text-neutral-600" size={32} />
          <p className="leading-relaxed">{error || 'No se pudo cargar el resumen. Verifica que hay un producto analizado.'}</p>
          
          {/* Botón discreto que aparece solo si el mensaje es exacto */}
          {error === "Aún no hay productos analizados. Carga un enlace para empezar." && (
            <button
              onClick={() => setMostrarModalProducto(true)}
              className="inline-flex items-center gap-2 px-4 py-2 mt-2 bg-neutral-900 border border-neutral-800 hover:bg-neutral-800 hover:text-neutral-300 text-neutral-400 text-xs font-semibold rounded-lg transition-all"
            >
              <PlusCircle size={14} />
      ...
      {/* Renderizamos el modal en este estado vacío[cite: 1] */}
      <ModalCargarProducto
  ...
  return (
    <>
      ...
      {/* Renderizamos el modal también en el estado normal, por si después quieres un botón extra aquí[cite: 1] */}
      <ModalCargarProducto
        estaAbierto={mostrarModalProducto}
        alCerrar={() => setMostrarModalProducto(false)}
        alCompletar={onProductoCargado}
